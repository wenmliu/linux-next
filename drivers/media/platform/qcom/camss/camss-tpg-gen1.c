...
static int tpg_stream_on(struct tpg_device *tpg)
{
	...
	for (vc = 0; vc < MSM_TPG_MAX_SRC_STREAMS; vc++) {
		if (!(tpg->en_vc & BIT(vc)))
			continue;

		last_vc = vc;

		input_format = &tpg->fmt[vc];
		...
		/* VC configuration */
		val = FIELD_PREP(TPG_VC_n_CFG0_NUM_ACTIVE_DT, tpg->res->dt_cnt - 1) |
...
static u32 tpg_hw_version(struct tpg_device *tpg)
{
	...
	dev_err_once(tpg->camss->dev, "tpg HW Version = %u.%u.%u\n",
